...
#include "core/cypher_string_t.h"
...
class ColumnVector {
 ...
 public:
    ...
    explicit ColumnVector(size_t element_size, size_t capacity = DEFAULT_VECTOR_CAPACITY)
        : element_size_(element_size),
          ...
          bitmask_(capacity),
          overflow_buffer_capacity_(1024),
          overflow_buffer_(new uint8_t[overflow_buffer_capacity_]),
          overflow_offset_(0) {}
    ...
    void CopyFromRowData(uint32_t pos, const uint8_t* row_data) {
        std::memcpy(data_.get() + pos * element_size_, row_data, element_size_);
    }

    void CopyToRowData(uint32_t pos, uint8_t* row_data) const {
    ...
    void* AllocateOverflow(uint64_t size) const {
        if (overflow_offset_ + size > overflow_buffer_capacity_) {
 ...
 private:
    ...
    mutable std::unique_ptr<uint8_t[]> overflow_buffer_;
...
class StringColumn {
 public:
    ...
    static void CopyToRowData(const ColumnVector* vector, uint32_t pos, uint8_t* rowData) {
        auto& srcStr = vector->GetValue<cypher_string_t>(pos);
        auto& dstStr = *reinterpret_cast<cypher_string_t*>(rowData);
        if (cypher_string_t::IsShortString(srcStr.len)) {
            dstStr.SetShortString(reinterpret_cast<const char*>(srcStr.prefix), srcStr.len);
        } else {
